...
var generateCallbacks = function(testResultId, operation, requests, callbackUrl) {
  ...
  var callbacks = {
    ...
    beforeIteration: function (err, cursor) {
        ...
        winstong.debug('beforeIteration: ' + JSON.stringify(err));
    },
    ...
    beforeRequest: function (err, cursor, request, item) {
        ...
        // item: sdk.Item
        // request: sdk.request
        winston.log('beforeRequest: ' + JSON.stringify(err));
    },
    ...
    request: function (err, cursor, response, request, item, cookies) {
        ...
        // response: sdk.Response
        // request: sdk.request
        winston.log('request: ' + JSON.stringify(err));
    },
          ...
          if (records[i][0] != undefined) {
            ...
            if (record.result != undefined) {
              ...
              for (var j=0; j<requests.length; j++) {
                ...
                winston.silly('Comparing record.result.request to ' + JSON.stringify(request));
                winston.silly(record.result.request.url + ' VS ' + request.endpointUrl);
                winston.silly(record.result.request.method + ' VS ' + request.method);
                if (request.endpointUrl == record.result.request.url
                    && request.method == record.result.request.method) {
                  winston.silly('Found a match')
